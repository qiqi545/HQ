...
{
    /// <summary>
    /// An implementation of <see cref="IObjectSaveService{T}" /> that defers access to an <see cref="IObjectSaveRepository{T}" />.
    /// </summary>
    public class ObjectSaveService<TObject> : IObjectSaveService<TObject> where TObject : IObject
    {
        ...
        public ObjectSaveService(IObjectSaveRepository<TObject> repository)
        {
        ...
        public virtual async Task SaveAsync(IEnumerable<TObject> objects, BatchSaveStrategy strategy, long startingAt = 0, int? count = null)
        {
    ...
    /// <summary>
    /// An implementation of <see cref="IObjectSaveService" /> that defers access to an <see cref="IObjectSaveRepository" />.
    /// </summary>
    public class ObjectSaveService : IObjectSaveService
    {
        private readonly IObjectSaveRepository _repository;

        public ObjectSaveService(IObjectSaveRepository repository)
        {
            _repository = repository;
        }
        ...
        public virtual async Task SaveAsync(Type type, IEnumerable<IObject> objects, BatchSaveStrategy strategy, long startingAt = 0, int? count = null)
        {
