...
namespace HQ.Flow.Tests
{
	public class MessageAggregatorTests
	{
		...
		public void Publishes_to_manifold_handler()
		{
			...
			ManifoldMessageHandler handler = new ManifoldMessageHandler();
		...
		public void Can_subscribe_with_manifold_hierarchical_consumer()
		{
			...
			ManifoldHierarchicalMessageHandler handler = new ManifoldHierarchicalMessageHandler();
			aggregator.Subscribe(handler);
		...
		public void Can_subscribe_with_multiple_hierarchical_consumers()
		{
			...
			ManifoldHierarchicalMessageHandler handler1 = new ManifoldHierarchicalMessageHandler();
			ManifoldHierarchicalMessageHandler handler2 = new ManifoldHierarchicalMessageHandler();
			ManifoldHierarchicalMessageHandler handler3 = new ManifoldHierarchicalMessageHandler();
		...
		public void Multiple_subscriptions_of_the_same_kind_dont_duplicate()
		{
			...
			var aggregator = new MessageHub();
			aggregator.Subscribe<StringMessage>(handler1);
			aggregator.Subscribe<StringMessage>(e => handled++);
			aggregator.Subscribe<StringMessage>(handler2);
		...
		public void Handlers_can_return_false_safely()
		{
			...
			bool result = hub.Publish(new InheritedMessage());
			Assert.Equal(1, handler.Handled);
			Assert.False(result);
		}
		...
		public void Two_handlers_for_the_same_event_type()
		{
			...
			hub.Subscribe(h1);
			hub.Subscribe(h2);
			bool result = hub.Publish(new StringMessage("value"));
		...
		public void Two_identical_handlers()
		{
			...
			hub.Subscribe((IMessageHandler<StringMessage>)h1);
			hub.Subscribe((IMessageHandler<StringMessage>)h2);
			bool result = hub.Publish(new StringMessage("value"));
			Assert.True(h1.Handled == 1);
			Assert.True(h2.Handled == 1);
			Assert.True(result);
