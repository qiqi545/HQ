...
{
    /// <summary>
    /// An implementation of <see cref="IObjectDeleteService{T}" /> that defers access to an <see cref="IObjectDeleteRepository{T}" />.
    /// </summary>
    public class ObjectDeleteService<TObject> : IObjectDeleteService<TObject> where TObject : IObject
    {
        ...
        public ObjectDeleteService(IObjectDeleteRepository<TObject> repository)
        {
    ...
    /// <summary>
    /// An implementation of <see cref="IObjectDeleteService" /> that defers access to an <see cref="IObjectDeleteRepository" />.
    /// </summary>
    public class ObjectDeleteService : IObjectDeleteService
    {
        private readonly IObjectDeleteRepository _repository;

        public ObjectDeleteService(IObjectDeleteRepository repository)
        {
            _repository = repository;
        }
        ...
        public virtual async Task DeleteAsync(Type type, IEnumerable<IObject> objects, long startingAt = 0, int? count = null)
        {
