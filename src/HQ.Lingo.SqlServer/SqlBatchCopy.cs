...
namespace HQ.Lingo.SqlServer
{
    public class SqlBatchCopy : IDataBatchOperation<SqlServerOptions>
    {
        ...
        public virtual SqlServerOptions Before(IDbConnection connection, IDataDescriptor descriptor, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var database = connection.Database;
            var settings = new SqlServerOptions
            {
                PageVerify = connection.Execute("SELECT page_verify_option_desc FROM sys.databases WHERE [NAME] = @Database", new { Database = database }, transaction, commandTimeout).ToString(),
                RecoveryModel = connection.Execute("SELECT recovery_model_desc FROM sys.databases WHERE [NAME] =  @Database", new { Database = database }, transaction, commandTimeout).ToString()
            };
        ...
        public virtual async Task ExecuteAsync<T>(IDbConnection connection, IDataDescriptor descriptor, IEnumerable<T> objects, long? startingAt, int? count = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            try
            {
                ...
                var reader = new ObjectReader(typeof(T), objects, descriptor.Inserted.Select(x => x.ColumnName).ToArray());
        ...
        public void After<T>(IDbConnection connection, IDataDescriptor descriptor, SqlServerOptions options, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var database = connection.Database;
            connection.Execute("USE master");
        ...
        public virtual BatchMap GenerateBulkCopyMapping(IDataDescriptor descriptor, IDataReader reader, IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null)
        {
        ...
        public virtual IEnumerable<string> GetDatabaseTableColumns(IDbConnection connection, string tableName, IDbTransaction transaction = null, int? commandTimeout = null)
        {
