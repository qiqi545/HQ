...
namespace Lime.Internal.UriTemplates
{
	...
	public class UriTemplate
	{
		...
		private readonly Uri _ComponentBaseUri = new Uri("https://localhost.com", UriKind.Absolute);
		...
		private Result ResolveResult()
		{
			...
			if (result.ErrorDetected) throw new ArgumentException("Malformed template : " + result);
			return result;
		}
		...
		private bool ProcessVariable(VarSpec varSpec, Result result, bool multiVariableExpression = false)
		{
			    ...
			    || _parameters[varname] is IDictionary && ((IDictionary) _parameters[varname]).Count == 0)
			{
				if (_resolvePartially)
				{
					...
					{
						if (varSpec.First) result.Append("{");
			...
			else
			{
				...
				if (list == null && value is IEnumerable<string>) list = ((IEnumerable<string>) value).ToList();
				;
				if (list != null)
				{
		...
		public IDictionary<string, object> GetParameters(Uri uri,
			QueryStringParameterOrder order = QueryStringParameterOrder.Strict)
		{
			switch (order)
			{
				case QueryStringParameterOrder.Strict:
				{
					if (_ParameterRegex == null)
					{
						...
						lock (this) _ParameterRegex = new Regex(matchingRegex);
					}
				...
				case QueryStringParameterOrder.Any:
				{
					...
					var uriString =
						uri.GetComponents(UriComponents.SchemeAndServer | UriComponents.Path | UriComponents.Fragment,
