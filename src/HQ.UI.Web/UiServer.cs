	...
	public class UiServer
	{
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			try
			{
				...
				services.AddUi(_env, uiAssemblies);
			}
			finally
			{
		...
		private static void StartServer(string[] args)
		{
			...
			var config = new ConfigurationBuilder()
				...
				.AddJsonFile("blowdart.json", true)
				.Build();
			...
			using (webHost)
			{
				...
				foreach (var assembly in settings.ComponentAssemblies)
				{
					...
					foreach (var type in types)
					{
						if (!Attribute.IsDefined(type, typeof(DeployOnAttribute)))
							continue;
						var deployOn =
							(DeployOnAttribute) Attribute.GetCustomAttribute(type, typeof(DeployOnAttribute));
				...
				appLifetime.ApplicationStopping.Register(obj => ((TaskCompletionSource<object>) obj).TrySetResult(null),
					completionSource);
...
");
		...
		public static void Start<TStartup, TService, TComponent, TModel>(string[] args)
		...
		public static void AddHandler(string template, MethodInfo method)
		{
			...
			{
				Trace.TraceWarning(
					$"Replacing existing handler for template '{template}'. Defer adding handlers until they represent a canonical set, to avoid this warning.");
		...
		public static void AddHandler(string template, string handlerNameOrMethodName)
		{
			...
			Trace.TraceWarning(
				$"Handler '{handlerNameOrMethodName}' was added by stack frame. You can explicitly define the handler name via HandlerNameAttribute, or the complete Handler definition via HandlerAttribute, to avoid this message.");
