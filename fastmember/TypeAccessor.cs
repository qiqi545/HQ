...
namespace fastmember
{
    ...
    public abstract class TypeAccessor
    {
        ...
        private static readonly Hashtable typeLookup = new Hashtable();
        ...
        public static TypeAccessor Create(Type type)
        {
            ...
            TypeAccessor obj = (TypeAccessor)typeLookup[type];
            if (obj != null) return obj;

            lock (typeLookup)
            {
                // double-check
                obj = (TypeAccessor)typeLookup[type];
                if (obj != null) return obj;
        ...
        private static void WriteGetter(ILGenerator il, TypeInfo type, PropertyInfo[] props, FieldInfo[] fields, bool isStatic)
        {
            LocalBuilder loc = type.IsValueType ? il.DeclareLocal(type.AsType()) : null;
        ...
        private static void WriteSetter(ILGenerator il, TypeInfo type, PropertyInfo[] props, FieldInfo[] fields, bool isStatic)
        {
            if (type.IsValueType)
            {
            ...
            else
            {
                ...
                foreach (PropertyInfo prop in props)
                {
                    ...
                    Cast(il, type, loc);
                    il.Emit(value);
                    Cast(il, prop.PropertyType.GetTypeInfo(), null);
                ...
                foreach (FieldInfo field in fields)
                {
                    ...
                    Cast(il, type, loc);
                    il.Emit(value);
                    Cast(il, field.FieldType.GetTypeInfo(), null);
        ...
        private static bool IsFullyPublic(TypeInfo typeInfo)
        {
            while (typeInfo.IsNestedPublic)
        ...
        static TypeAccessor CreateNew(Type type)
        {
            ...
            if (!IsFullyPublic(typeInfo))
            {
                ...
                WriteGetter(dynGetter.GetILGenerator(), typeInfo, props, fields, true);
                WriteSetter(dynSetter.GetILGenerator(), typeInfo, props, fields, true);
                ...
                var delegateAccessor = new DelegateAccessor(
                    ...
                    (Func<object>) dynCtor?.CreateDelegate(typeof (Func<object>)))
                {
                    PropertyInfos = props,
                    FieldInfos = fields
                };
            ...
            TypeBuilder tb = module.DefineType("FastMember_dynamic." + type.Name + "_" + Interlocked.Increment(ref counter), (typeof(TypeAccessor).GetTypeInfo().Attributes | TypeAttributes.Sealed) & ~TypeAttributes.Abstract, typeof(TypeAccessor));
            ...
            WriteGetter(il, typeInfo, props, fields, false);
            ...
            il = body.GetILGenerator();
            WriteSetter(il, typeInfo, props, fields, false);
            ...
            var typeAccessor = (TypeAccessor)Activator.CreateInstance(tb.CreateTypeInfo().AsType());
            typeAccessor.PropertyInfos = props;
            typeAccessor.FieldInfos = fields;
            return typeAccessor;
        }
        ...
        private static void Cast(ILGenerator il, TypeInfo type, LocalBuilder addr)
        {
            if (type.AsType() == typeof(object)) { }
            else if (type.IsValueType)
            {
                il.Emit(OpCodes.Unbox_Any, type.AsType());
                if (addr != null)
                {
            ...
            {
                il.Emit(OpCodes.Castclass, type.AsType());
            }
        }
