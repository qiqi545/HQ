...
namespace metrics.Core
{
    public abstract class TimerMetricBase : IMetric, IMetered
    {
        ...
        protected TimerMetricBase(TimeUnit durationUnit, TimeUnit rateUnit)
            : this(
                durationUnit, rateUnit, MeterMetric.New("calls", rateUnit),
                new HistogramMetric(HistogramMetric.SampleType.Biased), true /* clear */)
        {
        }

        protected TimerMetricBase(TimeUnit durationUnit, TimeUnit rateUnit, MeterMetric meter, HistogramMetric histogram,
            bool clear)
        {
        ...
        {
            get { return Histogram.Values.Select(value => ConvertFromNanos(value)).ToList(); }
        }
